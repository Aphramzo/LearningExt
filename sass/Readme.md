...
folder are mapped to the application's root namespace, 'LearningExt'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "LearningExt"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'LearningExt' in the same way
as `"LearningExt/sass/src"`.
